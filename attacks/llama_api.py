...
class LlamaAPI():
    ...
    def build(
        ...
        return LlamaAPI(model, tokenizer)
    ...
    def generate(
        ...
        if logprobs:
            token_logprobs = torch.zeros((bsz, total_len, self.tokenizer.n_words), dtype=torch.float)
        ...
        for cur_pos in range(min_prompt_len, total_len):
            ...
            if logitbias:
                logits[:, -1, list(logitbias.keys())] += torch.tensor(list(logitbias.values()))
            ...
            if logprobs:
                token_logprobs[:, prev_pos + 1 : cur_pos + 1] = torch.log_softmax(logits, dim=-1)
        ...
        if logprobs:
            token_logprobs = torch.topk(token_logprobs, k)
        ...
        for i, toks in enumerate(tokens.tolist()):
            ...
            if logprobs:
                probs = {
                    "values": token_logprobs.values[i, start : len(prompt_tokens[i]) + max_gen_len],
                    "tokens": token_logprobs.indices[i, start : len(prompt_tokens[i]) + max_gen_len]
                }
